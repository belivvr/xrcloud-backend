...
if [ "$ENV" = "prod" ]; then
    ...
    docker run --restart always --name $TYPEORM_HOST -d \
...
# docker exec $TYPEORM_HOST psql -U $TYPEORM_USERNAME -c "CREATE DATABASE \"$TYPEORM_DATABASE\";"
# docker exec $TYPEORM_HOST psql -U $TYPEORM_USERNAME -d $TYPEORM_DATABASE -c "CREATE USER \"$TYPEORM_USERNAME\" WITH PASSWORD '$TYPEORM_PASSWORD';"
